...
import io.ubiqesh.uplink.vertx.event.changelog._
...
import io.ubiqesh.uplink.vertx.rpc.{Rpc, Param, Method}
 ...
 * @param server the
 */
class Endpoint(var socket: ServerWebSocket, var persistence: Persistence, var server: UplinkServer)
    ...
    if (open) {
      ...
      if (event.getType == StateChangeEventType.EVENT) {
        ...
        if (`object`.isInstanceOf[JsonObject]) {
          ...
          io.ubiqesh.uplink.vertx.json.Node.populate(node, `object`.asInstanceOf[String])
