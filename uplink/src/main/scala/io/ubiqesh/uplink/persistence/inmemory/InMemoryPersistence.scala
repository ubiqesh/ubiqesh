package io.ubiqesh.uplink.persistence.inmemory

import io.ubiqesh.uplink.vertx.UplinkServer
...
import io.ubiqesh.uplink.vertx.event.changelog.ChangeLogBuilder
import io.ubiqesh.uplink.vertx.json.NodeVisitor
import io.ubiqesh.uplink.persistence.Persistence
import io.ubiqesh.uplink.persistence.queries.QueryEvaluator

import scala.collection.JavaConversions._
import org.vertx.java.core.json.JsonObject

class InMemoryPersistence(private var ubiqesh: UplinkServer) extends Persistence {
  ...
  override def get(path: Path): AnyRef = {
    if (path == null || path.isEmtpy) {
      ...
      io.ubiqesh.uplink.vertx.json.Node.getObjectForPath(model, path)
    }
  }

  override def getJsonObject(path: Path): JsonObject = {
    val log = new ChangeLog()
    val nodeForPath = io.ubiqesh.uplink.vertx.json.Node.getNodeForPath(model, log, path)
    ubiqesh.distributeChangeLog(log)
    nodeForPath
  }
  ...
  override def remove(log: ChangeLog, path: Path) {
    ...
    val parentNode = io.ubiqesh.uplink.vertx.json.Node.getNodeForPath(model, log, parentPath)
    val node = io.ubiqesh.uplink.vertx.json.Node.getNode(parentNode, nodeName)
    val parent = io.ubiqesh.uplink.vertx.json.Node.getNodeForPath(model, log, parentPath)
    io.ubiqesh.uplink.vertx.json.Node.accept(node, path, new ChildRemovedSubTreeVisitor(log))
  ...
  override def syncPath(path: Path, handler: Endpoint) {
    val log = new ChangeLog()
    val node = io.ubiqesh.uplink.vertx.json.Node.getNodeForPath(model, log, path)
  ...
  override def syncPropertyValue(path: Path, handler: Endpoint) {
    val log = new ChangeLog()
    val node = io.ubiqesh.uplink.vertx.json.Node.getNodeForPath(model, log, path.getParent)
  ...
  override def updateValue(log: ChangeLog,
    ...
    var created = false
    if (!io.ubiqesh.uplink.vertx.json.Node.pathExists(model, path)) {
      created = true
    }
    val parent = io.ubiqesh.uplink.vertx.json.Node.getNodeForPath(model, log, path.getParent)
    if (payload.isInstanceOf[JsonObject]) {
      ...
      io.ubiqesh.uplink.vertx.json.Node.populate(new ChangeLogBuilder(log, path, path.getParent, node), node, payload.asInstanceOf[JsonObject])
      if (created) {
  ...
  override def applyNewValue(log: ChangeLog,
    ...
    try {
      ...
      while (!currentPath.isSimple) {
        log.addValueChangedLogEntry(currentPath.getLastElement, currentPath, currentPath.getParent, io.ubiqesh.uplink.vertx.json.Node.getObjectForPath(model, currentPath))
  ...
  private def addChangeEvent(log: ChangeLog, path: Path) {
    val payload = io.ubiqesh.uplink.vertx.json.Node.getObjectForPath(model, path)
    val parent = io.ubiqesh.uplink.vertx.json.Node.getNodeForPath(model, log, path.getParent)
    log.addChildChangedLogEntry(path.getLastElement, path.getParent, path.getParent.getParent, payload,
