package io.ubiqesh.edge.persistence.inmemory
...
import io.ubiqesh.edge.vertx.event.changelog.ChangeLogBuilder
import io.ubiqesh.edge.vertx.json.NodeVisitor
...
class InMemoryPersistence(private var ubiqesh: EdgeServer) extends Persistence {
  ...
  override def get(path: Path): AnyRef = {
    if (path == null || path.isEmtpy) {
      ...
      io.ubiqesh.edge.vertx.json.Node.getObjectForPath(model, path)
    }
  }

  override def getJsonObject(path: Path): JsonObject = {
    val log = new ChangeLog()
    val nodeForPath = io.ubiqesh.edge.vertx.json.Node.getNodeForPath(model, log, path)
  ...
  override def remove(log: ChangeLog, path: Path) {
    ...
    val parentNode = io.ubiqesh.edge.vertx.json.Node.getNodeForPath(model, log, parentPath)
    val node = io.ubiqesh.edge.vertx.json.Node.getNode(parentNode, nodeName)
    val parent = io.ubiqesh.edge.vertx.json.Node.getNodeForPath(model, log, parentPath)
    io.ubiqesh.edge.vertx.json.Node.accept(node, path, new ChildRemovedSubTreeVisitor(log))
  ...
  override def syncPath(path: Path, handler: Endpoint) {
    val log = new ChangeLog()
    val node = io.ubiqesh.edge.vertx.json.Node.getNodeForPath(model, log, path)
  ...
  override def syncPropertyValue(path: Path, handler: Endpoint) {
    val log = new ChangeLog()
    val node = io.ubiqesh.edge.vertx.json.Node.getNodeForPath(model, log, path.getParent)
  ...
  override def updateValue(log: ChangeLog,
    ...
    var created = false
    if (!io.ubiqesh.edge.vertx.json.Node.pathExists(model, path)) {
      created = true
    }
    val parent = io.ubiqesh.edge.vertx.json.Node.getNodeForPath(model, log, path.getParent)
    if (payload.isInstanceOf[JsonObject]) {
      ...
      io.ubiqesh.edge.vertx.json.Node.populate(new ChangeLogBuilder(log, path, path.getParent, node), node, payload.asInstanceOf[JsonObject])
      if (created) {
  ...
  override def applyNewValue(log: ChangeLog,
    ...
    try {
      ...
      while (!currentPath.isSimple) {
        log.addValueChangedLogEntry(currentPath.getLastElement, currentPath, currentPath.getParent, io.ubiqesh.edge.vertx.json.Node.getObjectForPath(model, currentPath))
  ...
  private def addChangeEvent(log: ChangeLog, path: Path) {
    val payload = io.ubiqesh.edge.vertx.json.Node.getObjectForPath(model, path)
    val parent = io.ubiqesh.edge.vertx.json.Node.getNodeForPath(model, log, path.getParent)
    log.addChildChangedLogEntry(path.getLastElement, path.getParent, path.getParent.getParent, payload,
