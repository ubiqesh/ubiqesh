...
Backbone.Ubiqesh = function(ref) {
  this._fbref = ref;
  this._children = [];
  if (typeof ref == "string") {
    this._fbref = new Ubiqesh(ref);
  }
  _.bindAll(this);
...
_.extend(Backbone.Ubiqesh.prototype, {
...
Backbone.Ubiqesh.sync = function(method, model, options, error) {
  var store = model.ubiqesh || model.collection.ubiqesh;
...
// Override 'Backbone.sync' to default to Ubiqesh sync.
// the original 'Backbone.sync' is still available in 'Backbone.oldSync'
Backbone.sync = function(method, model, options, error) {
  var syncMethod = Backbone.oldSync;
  if (model.ubiqesh || (model.collection && model.collection.ubiqesh)) {
    syncMethod = Backbone.Ubiqesh.sync;
  }
  return syncMethod.apply(this, [method, model, options, error]);
};

// Custom Ubiqesh Collection.
Backbone.Ubiqesh.Collection = Backbone.Collection.extend({
  sync: function() {
    this._log("Sync called on a Ubiqesh collection, ignoring.");
  },

  fetch: function() {
    this._log("Fetch called on a Ubiqesh collection, ignoring.");
  },
  ...
  constructor: function(models, options) {
    ...
    if (options && options.ubiqesh) {
      this.ubiqesh = options.ubiqesh;
    }
    switch (typeof this.ubiqesh) {
      case "object": break;
      case "string": this.ubiqesh = new Ubiqesh(this.ubiqesh); break;
      case "function": this.ubiqesh = this.ubiqesh(); break;
      default: throw new Error("Invalid ubiqesh reference created");
    }

    // Add handlers for remote events.
    this.ubiqesh.on("child_added", _.bind(this._childAdded, this));
    this.ubiqesh.on("child_moved", _.bind(this._childMoved, this));
    this.ubiqesh.on("child_changed", _.bind(this._childChanged, this));
    this.ubiqesh.on("child_removed", _.bind(this._childRemoved, this));

    // Add handlers for all models in this collection, and any future ones
    // that may be added.
    function _updateModel(model, options) {
      this.ubiqesh.ref().child(model.id).update(model.toJSON());
    }
  ...
  add: function(models, options) {
    ...
    for (var i = 0; i < parsed.length; i++) {
      var model = parsed[i];
      this.ubiqesh.ref().child(model.id).set(model, _.bind(options.success, model));
    }
  },
  ...
  remove: function(models, options) {
    ...
    for (var i = 0; i < parsed.length; i++) {
      var model = parsed[i];
      this.ubiqesh.ref().child(model.id).set(null, _.bind(options.success, model));
    }
  },
  ...
  _parseModels: function(models) {
    ...
    for (var i = 0; i < models.length; i++) {
      ...
      if (!model.id) {
        model.id = this.ubiqesh.ref().push().name();
      }
      ret.push(model);
    }
    return ret;
...
// Custom Ubiqesh Model.
Backbone.Ubiqesh.Model = Backbone.Model.extend({
  save: function() {
    this._log("Save called on a Ubiqesh model, ignoring.");
  },

  destroy: function(options) {
    // TODO: Fix naive success callback. Add error callback.
    this.ubiqesh.ref().set(null, this._log);
  ...
  constructor: function(model, options) {
    ...
    // Add handlers for remote events.
    this.ubiqesh.on("value", this._modelChanged.bind(this));
  ...
  _updateModel: function(model, options) {
    ...
    // so Ubiqesh properly deletes them.
    var modelObj = model.toJSON();
    _.each(model.changed, function(value, key) {
      if (typeof value === "undefined" || value === null)
        modelObj[key] = null;
    });
    this.ubiqesh.ref().update(modelObj, this._log);
  },
