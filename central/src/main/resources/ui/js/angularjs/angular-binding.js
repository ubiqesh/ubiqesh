...
// Define the `ubiqesh` module under which all AngularUbiqesh services
// will live.
angular.module("ubiqesh", []).value("Ubiqesh", Ubiqesh);

// Define the `angularUbiqesh` service for implicit syncing.
// `angularUbiqesh` binds a
// model to $scope and keeps the data synchronized with a Ubiqesh location
// both ways.
angular.module("ubiqesh").factory("angularUbiqesh", [ "$q", "$parse", "$timeout", function($q, $parse, $timeout) {
	// The factory returns a new instance of the `AngularUbiqesh` object,
	// defined
	// below, everytime it is called. The factory takes 4 arguments:
	//
	// * `ref`: A Ubiqesh URL or reference. A reference with limits
	...
	// (or is present on the Ubiqesh URL provided). Pass in
	// `{}` for Object, `[]` for Array (default), `""` for
	// String and `true` for Boolean.
	return function(ref, scope, name, type) {
		var af = new AngularUbiqesh($q, $parse, $timeout, ref);
		return af.associate(scope, name, type);
	};
} ]);

// The `AngularUbiqesh` object that implements implicit synchronization.
function AngularUbiqesh($q, $parse, $timeout, ref) {
	this._q = $q;
	this._parse = $parse;
	...
	// `ref` can either be a string (URL to a Ubiqesh location), or a
	// `Ubiqesh` object.
	if (typeof ref == "string") {
		this._fRef = new Ubiqesh(ref);
	} else {
		this._fRef = ref;
	}
}
AngularUbiqesh.prototype = {
	// This function is called by the factory to create a new 2-way binding
	// between a particular model in a `$scope` and a particular Ubiqesh
	// location.
	associate : function($scope, name, type) {
		...
		// [Ticket #25](https://github.com/ubiqesh/angularUbiqesh/issues/25).
	...
	// [pull request
	// #34](https://github.com/ubiqesh/angularUbiqesh/pull/34).
	// This function is provided to the promise returned by `angularUbiqesh`
...
// Define the `angularUbiqeshCollection` service for explicit syncing.
// `angularUbiqeshCollection` provides a collection object that you can
// modify.
// [Original
// code](https://github.com/petebacondarwin/angular-ubiqesh/blob/master/ng-ubiqesh-collection.js)
// by @petebacondarwin.
angular.module("ubiqesh").factory("angularUbiqeshCollection", [ "$timeout", function($timeout) {
	return function(collectionUrlOrRef, initialCb) {
		// An internal representation of a model present in the collection.
		function angularUbiqeshItem(ref, index) {
			this.$ref = ref.ref();
			this.$id = ref.name();
			this.$index = index;
			angular.extend(this, {
		...
		// The provided ref can either be a string (URL to a Ubiqesh location)
		// or an object of type `Ubiqesh`. Ubiqesh objects with limits or
		// queries applies may also be provided.
		var collectionRef;
		if (typeof collectionUrlOrRef == "string") {
			collectionRef = new Ubiqesh(collectionUrlOrRef);
		} else {
		...
		collectionRef.on("child_added", function(data, prevId) {
			$timeout(function() {
				...
				addChild(index, new angularUbiqeshItem(data, index));
				updateIndexes(index);
			});
		});
		...
		collectionRef.on("child_changed", function(data, prevId) {
			$timeout(function() {
				...
				var item = new angularUbiqeshItem(data, index);

				updateChild(index, item);
			});
		});

		// `angularUbiqeshCollection` exposes three methods on the collection
		// returned.

		// Add an object to the remote collection. Adding an object is the
		// equivalent of calling `push()` on a Ubiqesh reference.
...
angular.module("ubiqesh").factory("angularUbiqeshQuery", [ "$timeout", function($timeout) {
