...
import akka.actor.{Props, ActorSystem, UntypedActor, ActorLogging}
import org.vertx.scala.core.http.{RouteMatcher, HttpServerRequest}

class DevicesEndpoint(prefix: String) extends RouteMatcher {
  ...
  all(prefix+"/devices/:deviceId", {
    request:HttpServerRequest => {
      devicesRestEndpoint.tell(request, null)
    }
  })
  all(prefix+"/devices/:deviceId/sensors", {
    request:HttpServerRequest => {
      devicesRestEndpoint.tell(request, null)
    }
  })
  all(prefix+"/devices/:deviceId/sensors/:sensorId", {
    request:HttpServerRequest => {
      devicesRestEndpoint.tell(request, null)
    }
  })
  all(prefix+"/devices/:deviceId/sensors/:sensorId/current", {
    request:HttpServerRequest => {
      devicesRestEndpoint.tell(request, null)
    }
  })
  all(prefix+"/devices/:deviceId/sensors/:sensorId/subscriptions", {
    request:HttpServerRequest => {
      devicesRestEndpoint.tell(request, null)
    }
  })
  all(prefix+"/devices/:deviceId/subscriptions", {
    request:HttpServerRequest => {
      devicesRestEndpoint.tell(request, null)
    }
  })
