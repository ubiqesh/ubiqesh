...
// Define the `yagni` module under which all AngularYagni services
// will live.
angular.module("yagni", []).value("Yagni", Yagni);

// Define the `angularYagni` service for implicit syncing.
// `angularYagni` binds a
// model to $scope and keeps the data synchronized with a Yagni location
// both ways.
angular.module("yagni").factory("angularYagni", [ "$q", "$parse", "$timeout", function($q, $parse, $timeout) {
	// The factory returns a new instance of the `AngularYagni` object,
	// defined
	// below, everytime it is called. The factory takes 4 arguments:
	//
	// * `ref`: A Yagni URL or reference. A reference with limits
	...
	// (or is present on the Yagni URL provided). Pass in
	// `{}` for Object, `[]` for Array (default), `""` for
	// String and `true` for Boolean.
	return function(ref, scope, name, type) {
		var af = new AngularYagni($q, $parse, $timeout, ref);
		return af.associate(scope, name, type);
	};
} ]);

// The `AngularYagni` object that implements implicit synchronization.
function AngularYagni($q, $parse, $timeout, ref) {
	this._q = $q;
	this._parse = $parse;
	...
	// `ref` can either be a string (URL to a Yagni location), or a
	// `Yagni` object.
	if (typeof ref == "string") {
		this._fRef = new Yagni(ref);
	} else {
		this._fRef = ref;
	}
}
AngularYagni.prototype = {
	// This function is called by the factory to create a new 2-way binding
	// between a particular model in a `$scope` and a particular Yagni
	// location.
	associate : function($scope, name, type) {
		...
		// [Ticket #25](https://github.com/yagni/angularYagni/issues/25).
	...
	// [pull request
	// #34](https://github.com/yagni/angularYagni/pull/34).
	// This function is provided to the promise returned by `angularYagni`
...
// Define the `angularYagniCollection` service for explicit syncing.
// `angularYagniCollection` provides a collection object that you can
// modify.
// [Original
// code](https://github.com/petebacondarwin/angular-yagni/blob/master/ng-yagni-collection.js)
// by @petebacondarwin.
angular.module("yagni").factory("angularYagniCollection", [ "$timeout", function($timeout) {
	return function(collectionUrlOrRef, initialCb) {
		// An internal representation of a model present in the collection.
		function angularYagniItem(ref, index) {
			this.$ref = ref.ref();
			this.$id = ref.name();
			this.$index = index;
			angular.extend(this, {
		...
		// The provided ref can either be a string (URL to a Yagni location)
		// or an object of type `Yagni`. Yagni objects with limits or
		// queries applies may also be provided.
		var collectionRef;
		if (typeof collectionUrlOrRef == "string") {
			collectionRef = new Yagni(collectionUrlOrRef);
		} else {
		...
		collectionRef.on("child_added", function(data, prevId) {
			$timeout(function() {
				...
				addChild(index, new angularYagniItem(data, index));
				updateIndexes(index);
			});
		});
		...
		collectionRef.on("child_changed", function(data, prevId) {
			$timeout(function() {
				...
				var item = new angularYagniItem(data, index);

				updateChild(index, item);
			});
		});

		// `angularYagniCollection` exposes three methods on the collection
		// returned.

		// Add an object to the remote collection. Adding an object is the
		// equivalent of calling `push()` on a Yagni reference.
...
angular.module("yagni").factory("angularYagniQuery", [ "$timeout", function($timeout) {
