...
Backbone.Yagni = function(ref) {
  this._fbref = ref;
  this._children = [];
  if (typeof ref == "string") {
    this._fbref = new Yagni(ref);
  }
  _.bindAll(this);
...
_.extend(Backbone.Yagni.prototype, {
...
Backbone.Yagni.sync = function(method, model, options, error) {
  var store = model.yagni || model.collection.yagni;
...
// Override 'Backbone.sync' to default to Yagni sync.
// the original 'Backbone.sync' is still available in 'Backbone.oldSync'
Backbone.sync = function(method, model, options, error) {
  var syncMethod = Backbone.oldSync;
  if (model.yagni || (model.collection && model.collection.yagni)) {
    syncMethod = Backbone.Yagni.sync;
  }
  return syncMethod.apply(this, [method, model, options, error]);
};

// Custom Yagni Collection.
Backbone.Yagni.Collection = Backbone.Collection.extend({
  sync: function() {
    this._log("Sync called on a Yagni collection, ignoring.");
  },

  fetch: function() {
    this._log("Fetch called on a Yagni collection, ignoring.");
  },
  ...
  constructor: function(models, options) {
    ...
    if (options && options.yagni) {
      this.yagni = options.yagni;
    }
    switch (typeof this.yagni) {
      case "object": break;
      case "string": this.yagni = new Yagni(this.yagni); break;
      case "function": this.yagni = this.yagni(); break;
      default: throw new Error("Invalid yagni reference created");
    }

    // Add handlers for remote events.
    this.yagni.on("child_added", _.bind(this._childAdded, this));
    this.yagni.on("child_moved", _.bind(this._childMoved, this));
    this.yagni.on("child_changed", _.bind(this._childChanged, this));
    this.yagni.on("child_removed", _.bind(this._childRemoved, this));

    // Add handlers for all models in this collection, and any future ones
    // that may be added.
    function _updateModel(model, options) {
      this.yagni.ref().child(model.id).update(model.toJSON());
    }
  ...
  add: function(models, options) {
    ...
    for (var i = 0; i < parsed.length; i++) {
      var model = parsed[i];
      this.yagni.ref().child(model.id).set(model, _.bind(options.success, model));
    }
  },
  ...
  remove: function(models, options) {
    ...
    for (var i = 0; i < parsed.length; i++) {
      var model = parsed[i];
      this.yagni.ref().child(model.id).set(null, _.bind(options.success, model));
    }
  },
  ...
  _parseModels: function(models) {
    ...
    for (var i = 0; i < models.length; i++) {
      ...
      if (!model.id) {
        model.id = this.yagni.ref().push().name();
      }
      ret.push(model);
    }
    return ret;
...
// Custom Yagni Model.
Backbone.Yagni.Model = Backbone.Model.extend({
  save: function() {
    this._log("Save called on a Yagni model, ignoring.");
  },

  destroy: function(options) {
    // TODO: Fix naive success callback. Add error callback.
    this.yagni.ref().set(null, this._log);
  ...
  constructor: function(model, options) {
    ...
    // Add handlers for remote events.
    this.yagni.on("value", this._modelChanged.bind(this));
  ...
  _updateModel: function(model, options) {
    ...
    // so Yagni properly deletes them.
    var modelObj = model.toJSON();
    _.each(model.changed, function(value, key) {
      if (typeof value === "undefined" || value === null)
        modelObj[key] = null;
    });
    this.yagni.ref().update(modelObj, this._log);
  },
